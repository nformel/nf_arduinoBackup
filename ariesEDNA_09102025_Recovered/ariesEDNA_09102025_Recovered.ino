//This code is specific to ARIES 4 for rotary valve offset
//LIBRARIES
#include <EEPROM.h>
...
#include <Encoder.h>
#include "MS5837.h"          //Library for Bar30 Depth and Temp sensor
...
#define stepsPerRevolution 200  //number of steps in one rotation, Stepper motor 23HS45-4204S1
#define gearRatio 10             //10:1 gear motor
#define valvePositions 16       //16 valve positions
...
#define stepperQuadEnc 4000 //Number of ticks by encoder per full revolution
#define stepperEncAPin 10
#define stepperEncBPin 11
Encoder stepperEnc(stepperEncAPin, stepperEncBPin);
int microstepping = 1;    // select microstepping mode, value is inverse e.g., 4 = 1/4
int sampleValve = 255;
...
int zeroSteps;          //Steps from zero position to valve 1
int totalSteps = stepperQuadEnc * gearRatio * microstepping;
int stepsPerValve = totalSteps / valvePositions;
int ccw_backLash = 800;  // backlash based on test 5/6/2025 and three valve positions (stdev <1)
int cw_backLash = 1000;  // backlash based on test 5/6/2025 and three valve positions (stdev ~50)
float offsetRatio = (0); // Was 16/800
// DRV8825 stepper(stepperSteps, stepperDir, stepperStep, stepperMode0, stepperMode1, stepperMode2);
int currentValvePosition[16] = { 0, 22, 45, 67, 90, 112, 135, 157, 180, 202, 225, 247, 270, 292, 315, 337 };  // Array to store valve positions
...
int eepromZeroStepsAddress = eepromWakeUpAddress + sizeof(time_t) + sizeof(currentValvePositionIndex);
...
static int last3wayValveState = -1;  // Remember last state
...
//int guiMenu = 0;             // An index value to establish which navigation menu is currently selected
...
void setup() {
  ...
  //pinMode(stepperPowerPin, OUTPUT);  //Set as output to power encoder on pumpA
  stepperEnc.write(0);
  ...
  if (digitalRead(reedSwitchPin) == LOW || digitalRead(reedSwitchPin) == HIGH) {  // Check if reed switch is triggered (GUI mode)
  //if (digitalRead(reedSwitchPin) == LOW) {  // Check if reed switch is triggered (GUI mode)
    menu = 0;
    activeMenu = 0;
    ...
    Wire2.begin();
    if (!sensor.init()) {  //make "while" if don't want code to keep going
    ...
    //OLED Setup
    setupOLED();
    // Read stored valve position from EEPROM
    ...
    if (currentValvePositionIndex < 0 || currentValvePositionIndex > 16) {  // Check if valve position is outside of 0-16
      ...
      Serial.println("Recommended: Perform a zeroing operation.");
      EEPROM.get(eepromValvePosAddress, currentValvePositionIndex);
      Serial.println("Stored valve position is now: " + String(currentValvePositionIndex));
    }
    // Print the retrieved or defaulted position
    Serial.println("Stored valve position: " + String(currentValvePositionIndex));
    return;
  }
    ...
    case 2:
      {
        ...
        EEPROM.get(eepromValvePosAddress, currentValvePositionIndex);
        Serial.println("Stored Valve Position: " + String(currentValvePositionIndex));
        EEPROM.get(eepromZeroStepsAddress, zeroSteps);
        Serial.println("Stored Zero Steps: " + String(zeroSteps));
        readSampleSettings();  //Read sample settings
        delay(3000);
        enterSamplingMode();
        break;  //Loop the Settings Menu navigation and display
...
// Function to set microstepping mode using your pin definitions
void setMicrostepping(int microstepping) {
  // Reset all pins to LOW before setting new mode
  digitalWrite(stepperMode0, LOW);
  digitalWrite(stepperMode1, LOW);
  digitalWrite(stepperMode2, LOW);
  // Set microstepping mode based on desired resolution
  switch (microstepping) {
    case 1:  // Full step
      digitalWrite(stepperMode0, LOW);
      digitalWrite(stepperMode1, LOW);
      digitalWrite(stepperMode2, HIGH);
      break;
    case 2:  // Non-circular half step (“a”)
      digitalWrite(stepperMode0, LOW);
      digitalWrite(stepperMode1, HIGH);
      digitalWrite(stepperMode2, LOW);
      break;
    case 4:  // 1/4 step
      digitalWrite(stepperMode0, LOW);
...
void adjustValvePosition() {
  ...
  if (currentValvePositionIndex == 0) {
    Serial.println("Is zero now, change to one.");
    currentValvePositionIndex = 1;
  }
  int currentValveSteps;
  int targetValveSteps;
  int stepsToRotate;
  int stepsToRotate2;
  ...
  if (selectedValvePositionIndex == currentValvePositionIndex) {
    ...
    currentValveSteps = stepsPerValve * currentValvePositionIndex;
    targetValveSteps = stepsPerValve * selectedValvePositionIndex;
    bool moveClockwise = (targetValveSteps > currentValveSteps);
    Serial.println(moveClockwise);
    if (moveClockwise == true) {
      digitalWrite(stepperDir, LOW); // Forward (CCW)
      stepsToRotate = round(abs(targetValveSteps - currentValveSteps));
      // Check if direction changed to forward (CCW)
      if (lastDirection == -1) {
        Serial.println("Direction changed to Forward (CCW), applying backlash.");
        stepsToRotate += ccw_backLash;  // Add backlash for CCW direction
      }
      lastDirection = 1;  // Update direction to forward (CCW)
    } else {
      digitalWrite(stepperDir, HIGH); // Backward (CW)
      stepsToRotate = round(abs(targetValveSteps - currentValveSteps));
      // Check if direction changed to backward (CW)
      if (lastDirection == 1) {
        Serial.println("Direction changed to Backward (CW), applying backlash.");
        stepsToRotate += cw_backLash;  // Add backlash for CW direction
      }
      lastDirection = -1;  // Update direction to backward (CW)
    }
  
  stepsToRotate2 = round(2416 + 1.035 * stepsToRotate);//This step is bespoke for ARIES 4 to try and fix an offset/scale issue on the rotary valve
  Serial.println("Current Valve Steps: " + String(currentValveSteps));
  Serial.println("Target Valve Steps: " + String(targetValveSteps));
  Serial.println("Final Steps to Rotate: " + String(stepsToRotate2));
  // Stepper movement setup 
  digitalWrite(stepperReset, HIGH);
  delay(50);
  setMicrostepping(microstepping);
  //digitalWrite(stepperPowerPin, HIGH);
  stepperEnc.write(0);  // Reset encoder reading
  // Calculate pump run time
  // Debugging line to monitor encoder readings
  Serial.print("Encoder goal: " + String(stepsToRotate2));
  while (abs(stepperEnc.read()) < stepsToRotate2) {
      digitalWrite(stepperStep, HIGH);
      delayMicroseconds(1000);
      digitalWrite(stepperStep, LOW);
      delayMicroseconds(1000);
      Serial.println(abs(stepperEnc.read()));
    }  
  // Stop the motor when the encoder count is reached
  Serial.println("Final Stepper encoder reading:");
  Serial.println(stepperEnc.read());
  //digitalWrite(stepperEncPowerPin, LOW);
  digitalWrite(stepperMode2, LOW);
  digitalWrite(stepperMode1, LOW);
  digitalWrite(stepperMode0, LOW);
  digitalWrite(stepperOn, LOW);
  // Update current position and save to EEPROM
  Serial.println("Saving new valve position to EEPROM...");
  EEPROM.update(eepromValvePosAddress, selectedValvePositionIndex);
  delay(100);  // Ensure EEPROM write completes before reading
  // **EEPROM TROUBLESHOOTING** - Verify the EEPROM write
  ...
  // Update the in-memory position tracking
...
void sampleValvePosition() {
  ...
  if (valveSelection == currentValvePositionIndex) {
    ...
    Serial.println("Not Zero.");
    currentValveSteps = stepsPerValve * currentValvePositionIndex;
    targetValveSteps = stepsPerValve * valveSelection;
    bool moveClockwise = (targetValveSteps > currentValveSteps);
    Serial.println(moveClockwise);
    if (moveClockwise == true) {
      digitalWrite(stepperDir, LOW); // Forward (CCW)
...
void moveValveOnePlus() {
  digitalWrite(stepperOn, HIGH);
  delay(50);
  int stepsToRotate = 80; //This translates to 800 encoder counts with 20 encoder counts per step
  ...
  delay(50);
  int adjSteps = stepsToRotate;  // Default movement
  stepperEnc.write(0);  // Reset encoder reading
  // Move the stepper motor
  for (int i = 0; i < adjSteps; i++) {
    digitalWrite(stepperStep, HIGH);
    delayMicroseconds(1000);
    digitalWrite(stepperStep, LOW);
    delayMicroseconds(1000);
...
void resetZeroPosition() {
  ...
  digitalWrite(stepperOn, HIGH);
  delay(50);
  digitalWrite(stepperDir, HIGH);  // Move towards home position CCW
  setMicrostepping(microstepping);
  int signalCount = 0;                        // Track number of HIGH signals
  int lastState = digitalRead(photoHomePin);  // Store initial sensor state
  ...
  while (signalCount < 7) {  // Stop on the 7th HIGH signal
    ...
    Serial.print(" - Photogate state: ");
    Serial.println(currentState);
    if (currentState == 1) {
      signalCount++;  // Increment the signal count
  ...
  digitalWrite(stepperMode1, LOW);
  digitalWrite(stepperMode0, LOW);
  currentValvePositionIndex = 0;  // Update the valve position index
  delay(50);
  digitalWrite(stepperOn, LOW);  // Turn off stepper motor
  lastDirection = -1; //Last valve direction was forward
  Serial.println("Last direction was CW.");
}
...
void zeroValveNow() {
  ...
  setMicrostepping(microstepping);
  int stepCount = 0;                          // Track total steps
  int signalCount = 0;                        // Track photogate triggers
  int lastState = digitalRead(photoHomePin);  // Store initial sensor state
  Serial.print("Photo Sensor Initial State: ");
  Serial.println(lastState);
  while (signalCount < 7) {  // Stop on the 7th HIGH signal
  ...
  Serial.println("7th photogate trigger reached. Stopping motor.");
  Serial.print("Total steps to zero: ");
  Serial.println(stepCount);
  // Save zeroSteps to EEPROM
  EEPROM.put(eepromZeroStepsAddress, stepCount);
  Serial.println("Zero steps saved in EEPROM.");
  // Update current position
  currentValvePositionIndex = 0;
  delay(50);
  digitalWrite(stepperMode2, LOW);
  digitalWrite(stepperMode1, LOW);
  digitalWrite(stepperMode0, LOW);
